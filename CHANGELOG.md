...
且默认零 LLM 成本零模拟漂移**——不搬全量工具模拟与 test-time scaling(那会引入"自信的错误
模拟",对治理系统比没有模拟更危险)。

- **执行前参数校验(规则层,确定性、默认开)** `core/simulate.py`:required/type/enum 不符 →
  **不执行、不入审批**,把纠错反馈回灌给 LLM(Gecko 的"失败-反馈-重试"闭环,安全版:非法调用
  绝不落地)。LLM 下一步据反馈自行修正,而非真实试错。
- **效果预览(默认确定性)**:改动性/需确认动作真实执行前生成"这次调用会造成什么"的预览,
  随 confirm 动作透传到 `/approvals` 待批项、通知与审计——把人在环的**盲批**升级为**看清后果再批**。
  工具自报 `preview`(确定性)优先,否则按 schema 生成摘要;内置 `remember`/`web_fetch`/`web_search`
  已带确定性预览。
- **可选 LLM 增强(默认关,需显式开启)**:①语义参数校验(schema 合法但语义不对,如 ticker 传
  公司全名而非 AAPL);②工具无自报预览时用 LLM 估计副作用(预览标注"估计")。校验器自身故障
  一律 **fail-open**(放行),不因判断器抖动误伤合法调用;安全由 auto/confirm/deny 与硬闸兜底。
- 接入三种自主形态(`tools`/`swarm`/`supervisor`)的工具循环,`simulator=None` 时行为与接入前
  完全一致(向后兼容)。新增 `config.simulation` 配置段(`core/config.py` + `config.yaml`)。
- 测试:`tests/test_m32_simulate.py` 14 例覆盖规则校验/语义校验/确定性与 LLM 预览/循环内非法参数
  回灌/预览经审批闸透传/向后兼容。**331 passed / 10 skipped**,ruff 全绿。
