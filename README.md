...
默认 `agent.autonomy=chat`(记忆增强问答)。设 `agent.autonomy=tools`(需 function-calling
模型,即 `llm.mode=api`/`litellm`)后,agent 会**自己决定调用工具**:检索/记忆(`recall`/
`remember`,内置)、上网(`web_search`/`web_fetch`,按 config 开)、以及任何第三方 `tool`
插件。每次工具调用都经**审批闸**(auto/confirm/deny + 全量审计)并受**循环硬上限**约束
(触顶记 `loop_capped`,不静默停)。`agent.tools` 选启用哪些。
